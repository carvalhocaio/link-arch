...
| `apps/web` | Application | Next.js 16 frontend: dashboard, my-links, auth pages |
...
| Auth | `POST /api/auth/sign-up`, `POST /api/auth/sign-in` | Public |
...
Client                     Elysia API                    Better Auth
  ...
  │                             │                              │
  │── POST /api/shorten ───────▶│                              │
  │   (with session cookie)     │── auth-middleware ──────────▶│
  │                             │◀── { user } ─────────────────│
...
`auth-middleware.ts` calls `auth.api.getSession()` on each request. If no valid session is found, it returns `401`. The authenticated user is attached to the Elysia context and passed to route handlers without re-fetching from the database.
...
- `hooks/use-url.ts` — shorten, list, update destination/key/status, delete
- `hooks/use-auth.ts` — sign-up, sign-in, sign-out, session
...
├── my-links/page.tsx     ← My Links (Server Component shell)
├── sign-in/page.tsx
├── sign-up/page.tsx
components/
├── app-topbar.tsx
