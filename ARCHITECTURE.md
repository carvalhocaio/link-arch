...
| Layer | Tool | Location | Scope | Runs in CI? |
|---|---|---|---|---|
| Unit | `bun test` | `apps/web/__tests__/unit/` | Pure functions: keygen, validator, expiry, dashboard metrics | Yes — every PR and push to main |
| Integration | `bun test` + real PostgreSQL | `apps/web/__tests__/integration/` | Every `url.service` operation against `link_arch_test` DB | Yes |
| Functional | `bun test` + module mocking | `apps/web/__tests__/functional/` | Route Handlers imported directly; auth mocked per test; IDOR guards asserted | Yes |
| E2E + Smoke | Playwright (Chromium, Firefox, WebKit, Pixel 5) | `apps/web/e2e/` | Full user flows, WCAG 2.1 AA accessibility via axe-core | Yes — `e2e.yml` |
| Load | k6 | `tests/load/redirect.js` | Redirect endpoint ramp to 200 VUs, p95 < 500ms | On-demand + weekly |
| Stress | k6 | `tests/load/shorten.js` | Shorten endpoint spike to 1000 VUs | On-demand + weekly |

**Test isolation:** All tests run against a dedicated `link_arch_test` database. Schema is applied via the same Drizzle migrations used in production. Data is truncated in `afterAll` hooks.

**Functional test mocking pattern:** Bun's `mock.module` is hoisted before imports so route handlers receive a stable mock at module load time. A mutable `authState` object allows changing the returned session per-test without re-registering the mock.

**Local execution:** `scripts/test-local.sh` starts a `postgres:16-alpine` Docker container, runs migrations, executes the requested test layer, and stops the container in a `trap EXIT` handler.
